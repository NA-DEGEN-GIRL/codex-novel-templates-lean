...
이 템플릿은 Codex native MCP와 Codex self-review를 기준으로 바로 쓰는 운영 분기다. 기본 supervision 구조는 `supervisor / writer / review` 3세션이다.
...
4. writer는 초안과 rewrite만 맡고, review는 post-write 검사와 summary 정합성을 맡는다.
5. supervisor와 review gate가 파일 기반으로 완료를 검증한다.
...
4. writer 세션이 초안 작성 + `EPISODE_META`까지 마무리
5. review 세션이 review floor에 맞는 post-write review 수행
6. review 세션이 `Immediate Carry-Forward` 포함 summary / review-log / action-log를 갱신
7. 필요 시 writer repair batch ↔ review recheck를 한 번 더 돌림
8. supervisor가 `verify-review-done.py` 통과 후 `/root/novel/config.json`에 화 정보를 반영
...
7. `batch-supervisor.md` 기준으로 writer 세션과 review 세션을 띄운다.
