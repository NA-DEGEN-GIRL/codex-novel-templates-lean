...
- Codex native MCP workflow + compatibility wrapper 포함
...
3. 모든 MCP를 등록해 사용할 수 있다고 가정하되, 기본 품질 체계는 Codex self-review와 audit 문서로 해결한다.
...
- `skills/`
  - Codex용 skill 정의
- `MCP`
  - `novel-editor`, `novel-calc`, `novel-hanja`, `novel-naming` native MCP 사용 가능
- `scripts/`
  - MCP 미등록/장애 시 쓰는 fallback CLI wrapper
...
- `patch-feasible`은 화수별 repair batch로 묶어 바로 수정
- `forward-fix`는 `scope(current-arc|next-arc)`와 `blocker`를 반드시 기록
- `scope: current-arc` open HOLD는 아크 마감 전에 해결
- 해결 후 `review-log`, `running-context`, `plot`을 함께 동기화
...
`scripts/`는 MCP 미등록, 일시 장애, 로컬 디버깅을 위한 fallback이다.
...
4. 필요 시 `scripts/compile-brief` fallback이 동작하는지 확인한다.
5. `batch-supervisor.md` 기준으로 writer 세션을 띄운다.
