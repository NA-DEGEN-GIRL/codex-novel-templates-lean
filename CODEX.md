...
4. 이번 화의 핵심 인물이 분명하면 `settings/03-characters.md`를 보고 대표 대사, 관계 상태, 관계별 말투 규칙을 확인한다. 대화 위계가 중요한 화면 `§8.1 호칭/어투 매트릭스`까지 함께 본다.
...
2. 본문은 항상 마크다운 제목 `# {N}화 - {제목}`으로 시작한다. 이 제목은 파일 끝 `EPISODE_META.title`과 동일해야 한다.
3. `settings/01-style-guide.md`를 따르고, 대화와 관계 장면은 `settings/03-characters.md`의 운용 앵커를 기준으로 변주한다. **규칙이 충돌하면 §0.8 우선순위를 따른다.**
4. 목표 분량은 {{TARGET_LENGTH}}를 기준으로 하되, 장면 밀도를 위해 필요한 길이를 확보한다.
5. 두 명 이상이 장면에 함께 있으면, 대화와 반응이 상황·관계·압력의 일부를 실제로 운반해야 한다. 지문이 이미 준 사실을 대사가 복창하는 식으로 때우지 않는다.
6. 같은 방, 식사 자리, 마차, 배, 숙영지, 은신처, 경계 장소에 있는 이름 있는 인물은 장면 논리에서 사라지면 안 된다. 침묵, 반응, 퇴장, 수면, 부재 중 하나로라도 자연스럽게 처리한다.
7. `novel-calc` MCP는 **검증용**이다. 수치를 서사의 엔진처럼 쓰지 않는다.
8. 계산 결과를 인물의 대사/독백/근접 시점 서술에 그대로 복붙하지 않는다. 사람처럼 어림하고, 감정 상태에 맞게 반올림하거나 감각적 표현으로 바꾼다.
9. 무협/역사/한자어 기반 작품이면 `novel-hanja` MCP를 사용한다. LLM 추정 한자 조립 금지.
10. 한자 병기는 첫 등장 또는 재등장 간격이 충분할 때만 쓴다. 이후에는 한국어 표기로 유지한다.
11. 시대에 맞는 단위/수사/어휘를 쓴다. 비현대 배경이면 아라비아 숫자를 본문에서 피한다.
12. 각주가 필요하면 `[^N]` 형식을 쓴다.
13. naming variant 검사가 필요하면 `novel-naming` MCP를 사용한다.
14. `settings/03-characters.md`의 대표 대사는 복붙용 문장이 아니라 운용 앵커다. 어휘 선택, 위계감, 판단 방식을 읽고 실제 장면에 맞게 변주한다.
15. 한국어 대화는 기본 캐릭터 톤보다 화자-청자 위계, 호칭, 거리, 공적/사적 상황을 먼저 맞춘다. 무뚝뚝한 인물이라도 부모, 연장자, 사부, 상급자 앞에서는 관계가 읽히는 표면 register를 남긴다.
16. 회상, 상대 시간 표현, 이동/회복/수련 기간, deadline이 있는 화는 초안 후 `settings/05-continuity.md`와 대조해 시간축 self-check를 수행한다. 애매하면 `novel-calc`의 날짜/기간 계산을 사용하거나 서술을 더 보수적으로 낮춘다.
...
7. dialogue voice drift — 주요 인물 대사가 `03-characters.md` 앵커와 `§8.1 호칭/어투 매트릭스`에서 이탈하지 않았는가? `dialogue-log.md`의 최근 톤 델타와 현재 화 일관성 확인. 역할 고착 판정은 `summaries/dialogue-log.md`를 직접 읽어 최근 5회 출연의 대화 기능을 확인한다 (brief에는 1건만 포함). 4/5회 동일 기능이면 역할 고착 경고.
...
- voice drift가 누적됐거나 `settings/01-style-guide.md` §0.3 대표 문단이 실제 화자 리듬과 어긋나면 `scripts/suggest-voice-profile-refresh.py` 후보를 보고 갱신하거나 HOLD로 넘긴다.
...
- `summaries/running-context.md` — open `forward-fix`가 있으면 `## HOLD 경고`에 다음 1~3화 안에서 반드시 기억해야 할 항목만 남긴다.
...
- `summaries/desire-state.md` — 독자 기대/불안이 명확히 움직였을 때 갱신
- `summaries/signature-moves.md` — opening/pressure/landing 패턴이 명확히 먹히거나 과사용 위험이 보일 때 갱신
...
6. batch-supervisor 경로에서는 sentinel 확인 뒤 `python3 <novel_dir>/scripts/verify-writer-done.py --novel-dir <novel_dir> --episode <N>`가 통과해야 다음 화로 넘어간다.
...
## 8. Dialogue Relationship Rules

> 이 절은 한국어 호칭, 존대/반말, 관계 거리감을 운영하기 위한 기준이다.
> 캐릭터의 "기본 말투"보다 화자-청자 관계와 위계가 먼저다.

### 8.1 호칭/어투 매트릭스

"행의 화자가 열의 청자에게 말할 때"의 기본 규칙을 기록한다. 자주 대화하는 쌍뿐 아니라, 아래 필수 축은 반드시 채운다.

| 화자 → 청자 | {{PARENT_OR_ELDER}} | {{PEER}} | {{SUBORDINATE}} | {{ENEMY_OR_RIVAL}} |
|-------------|---------------------|----------|------------------|--------------------|
| **{{PROTAGONIST}}** | {{존/반 + 호칭}} | {{존/반 + 호칭}} | {{존/반 + 호칭}} | {{존/반 + 호칭}} |
| **{{PARENT_OR_ELDER}}** | {{존/반 + 호칭}} | {{존/반 + 호칭}} | {{존/반 + 호칭}} | {{존/반 + 호칭}} |
| **{{PEER}}** | {{존/반 + 호칭}} | {{존/반 + 호칭}} | {{존/반 + 호칭}} | {{존/반 + 호칭}} |

> 필수 명시 조합: 부모/가족, 연장자/사부/상급자, 동년배/친한 사이, 아랫사람/후배, 적대자/대치 대상.
> 미등록 조합 기본값은 주변 인물 같은 진짜 예외에만 쓴다.
> `{{DEFAULT_RULE}}`처럼 한 줄로 끝내지 말고, 작품에서 자주 쓰일 관계축은 표에 직접 적는다.

### 8.2 상황별 어투 전환

| 캐릭터 | 공적 상황 | 사적 상황 | 감정 격화 시 |
|--------|-----------|-----------|--------------|
| {{CHAR}} | {{예: 단정한 존대}} | {{예: 짧은 반말}} | {{예: 더 짧아짐 / 일시적 존대 붕괴}} |
...
1. 기본 캐릭터 톤보다 화자-청자 위계와 관계가 우선한다.
2. 부모, 연장자, 사부, 상급자 앞에서는 "무뚝뚝하다"를 이유로 기계적 반말을 허용하지 않는다. 말수가 적어도 관계가 읽혀야 한다.
3. 어투 변화는 사건에 의해서만 발생한다. 설명 없는 갑작스러운 전환은 금지한다.
4. 감정 극단 시 일시적 이탈은 허용되지만, 장면 종료 후 기본 규칙으로 복귀해야 한다.
5. 공적/사적 상황은 같은 관계쌍이라도 다를 수 있다.
6. 대화는 정보 전달 외에 최소 하나의 기능을 수행해야 한다. 관계 확인, 감정 압력, 서브텍스트, 유머 중 하나가 읽혀야 한다.

## 9. Operating Standard
