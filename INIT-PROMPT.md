...
- 프로젝트 폴더는 반드시 `/root/novel/no-title-XXX` 형식으로 생성하라.
- `XXX`는 3자리 숫자다. 이미 같은 번호가 있으면 다음 사용 가능한 번호를 선택하라.
- 1단계에서는 컨셉 3개만 제안하고 반드시 멈춰라.
- 사용자가 컨셉 1개를 고르기 전에는 2~5단계로 진행하지 마라.
- 사용자의 선택을 받은 뒤에만 2단계 확장과 3~5단계를 이어서 수행하라.
...
- batch supervision은 `supervisor / writer / review` 3세션 구조를 기본으로 설계하라.
- 아크 경계 감사와 정기 점검이 문서 레벨에서 빠지지 않게 구성하라.
- 상위 카탈로그 `/root/novel/config.json`을 사용하는 환경을 기본 전제로 둔다.
...
3. 프로젝트 생성
   - 새 폴더 생성 (`/root/novel/no-title-XXX` 형식, 사용 가능한 다음 번호 선택)
   ...
   - `/root/novel/config.json`에 새 소설을 등록한다. 최소 필드: `id`, `title`, `author`, `genre`, `description`, `cover`, `fullStory`, `status`, `totalEpisodes: 0`, `parts`
   - `config.json`의 `parts`는 `ARC_MAP`와 같은 순서로 만든다. 각 part는 최소 `{ "name": "표시명", "episodes": [] }`
   ...
   - `scripts/run-codex-writer`, `scripts/run-codex-review`, `scripts/run-codex-supervisor`, `scripts/run-codex-auditor`, `scripts/tmux-send-codex`, `scripts/tmux-wait-sentinel`, `scripts/verify-writer-done.py`, `scripts/verify-review-done.py`가 usable 한지 확인한다
...
5. 보고
   - 생성한 프로젝트 경로 (`/root/novel/no-title-XXX`)
   - 생성한 파일 목록
   - `/root/novel/config.json` 등록 결과
   - 남은 리스크
   - 바로 집필 가능한지 여부
   - 첫 집필 전 추천 실행 순서
