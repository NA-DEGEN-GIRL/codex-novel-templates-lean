...
writer 세션은 codex-write-XXX이고, 이번 범위는 {START_EP}~{END_EP}화다.
supervisor가 review, summary 정합성 확인, /root/novel/config.json 업데이트까지 마무리해라.

중요:
- writer 세션에 `Working`, `Reading`, `Explored`, `Edited`, `Ran` 같은 진행 표시가 보이면 정상 진행으로 간주하고 추가 지시를 넣지 마라.
- 매 화 또는 repair batch를 보낼 때마다 고유한 `RUN_NONCE`를 새로 정하고, `WRITER_DONE ... :: run={RUN_NONCE}` exact sentinel을 그 실행에만 사용해라.
- `WRITER_DONE ... :: run={RUN_NONCE}` exact sentinel 또는 명시적 timeout 전에는 회복 프롬프트를 보내지 마라.
- 긴 멀티라인 프롬프트는 paste 후 첫 `Enter`와 1회 재시도 뒤에도 제출이 안 될 수 있다. 마지막 입력 프롬프트 줄이 그대로 남아 있으면 `Enter`를 1~2회 더 보내고 다시 확인해라.
```
...
7. supervisor는 완료 검증 후 `/root/novel/config.json`을 직접 갱신한다. writer는 `config.json`을 건드리지 않는다.
...
- 관련 있을 때만 `desire-state.md`, `signature-moves.md`
...
- 모든 본문/summary/action-log 작업이 끝난 뒤 마지막 줄에만 정확히 `WRITER_DONE chapter-{NN}.md :: run={RUN_NONCE}`를 1회 출력한다.
...
- `config.json` 갱신은 `chapter 파일 존재 + 필수 summary 갱신 + action-log 기록`이 확인된 뒤에만 수행한다.
...
- supervisor는 한 화의 chapter 작성 + summary 갱신 + action-log 기록 + `config.json` 반영이 모두 확인된 뒤에만 다음 화 프롬프트를 보낸다.
