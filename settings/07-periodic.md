...
- `plot/arc-XX.md`의 에피소드 항목에 `risk:` 태그가 있으면 조기 실행을 우선 검토
- 다음 화 오프닝에서 직전 화 공개 정보/부상/결정 결과를 어긴 carry-forward 문제가 보이면 즉시 점검
...
- `risk:` 태그 또는 prose drift override가 있으면 최소 `standard`까지 승격 가능
...
| P5.5 | carry-forward integrity | Core | `Immediate Carry-Forward`와 실제 다음 화 오프닝이 충돌하지 않는지 확인 |
